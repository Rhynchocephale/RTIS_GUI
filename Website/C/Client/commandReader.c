...
#define CONFIG_FILE 			"rtis_config.cfg"
#define TIMEOUT 				20
...
INT32 main(int argc, char *argv[])
{
	...
	*********************************************************************
	*/
	iCommandStatus = gl_Shm_Open(RTIS_KEY_PROC_TABLE, &pProcTbl, &Error);
	while (iCommandStatus == GL_ERROR)
	{
	...
	iCommandStatus = gl_Shm_Open(RTIS_KEY_MON_TABLE, &pMonTbl, &Error);
	while (iCommandStatus == GL_ERROR)
	{
	...
	iCommandStatus = gl_Shm_Open(RTIS_KEY_INTERFACE_TABLE, &pInterfaceTbl, &Error);
	while (iCommandStatus == GL_ERROR)
	{
	...
	*********************************************************************
	*/
	iCommandStatus = gl_Socket_Server_Connect(&tcpSocket, GL_DEVICE_TYPE_TCP_CLIENT_SOCKET, GUI_IP, DEFAULT_CONNECTION_PORT, GL_DEVICE_PROTOCOL_SATREF, GL_DEVICE_FORMAT_BINARY, TIMEOUT, GL_DEVICE_BLOCKED, &Error);
	...
	*********************************************************************
	*/
	iDataStatus = gl_Socket_Server_Connect(&udpSocket, GL_DEVICE_TYPE_UDP_CLIENT_SOCKET, GUI_IP, DEFAULT_CONNECTION_PORT+1, GL_DEVICE_PROTOCOL_SATREF, GL_DEVICE_FORMAT_BINARY, TIMEOUT, GL_DEVICE_BLOCKED, &Error);
	...
	*/
	while(1){
		...
		iCommandStatus = gl_Satref_ReadMsg(&tcpSocket, &satrefMsg, &Error);
		if(iCommandStatus != GL_OK){
		...
		*/
			...
			iCommandStatus = system("clear");
			for(n=0; n<RTIS_MAX_NO_OF_MAIN_PROC;n++)
			{
				iCommandStatus = gl_Shm_Get(pProcTbl, &procTableInfo, n, &Error);
				if (iCommandStatus == GL_ERROR)
					goto Proc_Init; //TODO
		...
		*/	
			...
			iCommandStatus = system("clear");
			iCommandStatus = gl_Shm_Get(pMonTbl, &monTableInfo, GL_SHM_SINGLE, &Error);
			if (iCommandStatus == GL_ERROR)
				goto Monitor_Init; //TODO

			iCommandStatus = gl_Shm_Get(pInterfaceTbl, &interfaceTableInfo, GL_SHM_SINGLE, &Error);
			if (iCommandStatus == GL_ERROR)
				goto Monitor_Init; //TODO
			...
			*/
			INT16_BIT16(&satrefMsg.HeaderData.sRefId,sRefId);
			satrefMsg.HeaderData.ucMsgClass = ucMsgClass;
			satrefMsg.HeaderData.ucMsgType = FIRST_MSG_TYPE+2;
			INT32_BIT32(&satrefMsg.HeaderData.lGpsSec,time(NULL));
			INT16_BIT16(&satrefMsg.HeaderData.sGpsmSec,0);
			INT16_BIT16(&satrefMsg.HeaderData.sLen,0);
			
			msg = gl_Satref_PackMsg(SatrefExtMsg, UINT8 *ucBuf, INT32 *iLen, &Error);
		...
		*/				
			...
			INT16_BIT16(&satrefMsg.HeaderData.sRefId,sRefId);
			satrefMsg.HeaderData.ucMsgClass = ucMsgClass;
			satrefMsg.HeaderData.ucMsgType = FIRST_MSG_TYPE+3;
			INT32_BIT32(&satrefMsg.HeaderData.lGpsSec,time(NULL));
			INT16_BIT16(&satrefMsg.HeaderData.sGpsmSec,0);
			INT16_BIT16(&satrefMsg.HeaderData.sLen,0);
		...
		*/				
			...
			if(myPid != 0){
				...
				// Open
				gl_RBuf_Open(gl_TKey Key, gl_TpRBuffer *pRBuf, &Error);  //TODO: real params

				while(1) {
					cmdReceived = time(NULL);
					while(time(NULL) - cmdReceived < ERROR_FETCH_TIME_LENGTH) {
						// Read
						gl_RBuf_Get(gl_TpRBuffer pRBuf, void *pData, &Error); //TODO: real params
						//TODO: SEND THIS IN A MESSAGE!
					}
					fetchError = 0;
